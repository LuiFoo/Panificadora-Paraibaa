  ...
  const loadProfileData = useCallback(async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setError(data.msg || "Erro ao carregar dados do usuário");
      }
    } catch (error) {
      ...
      <ProtectedRoute requiredPermission="administrador" redirectTo="/">
        ...
        <main className="min-h-screen bg-gray-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-md p-8 text-center max-w-md mx-4">
            ...
            <p className="text-gray-600 mb-6">Os dados deste usuário não foram encontrados.</p>
            <button
              onClick={() => router.back()}
              className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors flex items-center gap-2"
            >
        ...
        <div className="max-w-4xl mx-auto px-4">
          <BreadcrumbNav 
            items={[
              ...
              { label: "Perfil do Usuário", icon: "👤", color: "orange" }
            ]}
          />
          {/* Cabeçalho */}
